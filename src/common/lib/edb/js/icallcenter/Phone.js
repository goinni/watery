...
//                    window.location = "./notify.html";
    ...
    dialout: function (phoneNum) {
        ...
        this._sendAction("Originate", {
            ...
            // console.dir(response);
            // 电话呼出回调
            dialoutCenterCallback && dialoutCenterCallback.call(response, response.Succeed, phoneNum);
    ...
    hangup: function () {
        ...
        this._sendAction('Hangup', {
            ...
            var json = response;
            // 主动挂机接口
            callCenterDoHangup && callCenterDoHangup.call(response, response.Succeed);
            console.dir(json);
            if (json.Succeed) {
    ...
    setBusy: function (isBusy, busyType) {
        ...
        self._sendAction('Busy', params, function (response, ioArgs) {
            ...
            if (res.Succeed) {
                ...
                //状态修改成功接口
                setBusyCallBack && setBusyCallBack.call(this,isBusy, busyType);
            } else {
                if (res.Expired) {
                    self._relogin();
                }
...
//	            self.destroy();
                ...
                loadedPhoneIsOk && loadedPhoneIsOk.call(phone);
            } else {
                _self.logonStatus = "logonFail";
                result = false;
            }
